function ingredients(data, tooltip) {
	...
	var wawaCompos = d3.select("#comp")

	wawaCompos.selectAll(".comp")
		.data(comp)
		.enter().append("div")
		.attr("class", "comp row")
		.html(function(d){
	...
	wawaCompos.selectAll(".comp").selectAll("label")
		.on("mouseover", function(d){ 
			d3.select(".wawaTopping."+this.className).classed("selected", true);
		})
		.on("mouseout", function(d){
			d3.selectAll(".wawaTopping").classed("selected", false);
		})
	...
	bar.append("rect")
		.attr("class", function(d){ return "wawaTopping " + d.label.split(" ").join("-") })
