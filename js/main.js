...
function plural(value){
    if(value <= 1){ return " Wawa Order" }
    else { return " Wawa Orders" }
}
...
function sheetLoaded(data) {
    ...
    data = data.feed.entry.map(function (entry) {
        return {
            ...
            vote:       entry['gsx$rating']['$t'],
            // type:       entry['gsx$type']['$t'],
            type:       entry['gsx$name']['$t'],
            withwho:    entry['gsx$name']['$t'],
            type2:      entry['gsx$type']['$t'],
            address:    entry['gsx$address']['$t'],
            city:       entry['gsx$city']['$t'],
            lat:        entry['gsx$lat']['$t'],
            longi:      entry['gsx$lon']['$t'],
            topping:    entry['gsx$topping']['$t'],
            when:       entry["gsx$when"]['$t'],
            how:        entry['gsx$foodorbevorother']['$t'],
            size:       entry['gsx$size']['$t'],
            timehour:   entry['gsx$hour']['$t'],
            ampm:       entry['gsx$ampm']['$t'],
            place:      entry['gsx$address']['$t']
    ...
    //Pizza Counter
    d3.select(".totWawa").text(data.length);
